...
import { cn, signalLabel, draftTypeLabel, formatCompactCurrency, cleanDealName, urgencyLevel } from "@/lib/utils";
...
function DealContextPanel({ account }: { account: AccountListItem }) {
  ...
  return (
    <div className="h-full flex flex-col bg-zinc-50 overflow-y-auto">
      {/* Deal header */}
      <div className="bg-white border-b border-zinc-100 px-6 py-4 flex-shrink-0">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <h2 className="text-[15px] font-semibold text-zinc-900 truncate" title={account.name}>{cleanDealName(account.name)}</h2>
            <div className="flex items-center gap-2.5 mt-1.5 flex-wrap">
              <span className="text-[12px] text-zinc-400">{account.stage}</span>
              {account.deal_amount ? (
                <span className="text-[12px] text-zinc-600">${account.deal_amount.toLocaleString()}</span>
              ) : null}
              {closeDate && (
              ...
              <span className="text-[12px] text-zinc-400 flex items-center gap-1 ml-auto">
      ...
      <div className="flex-1 p-5 space-y-5">
        ...
        <div>
          ...
          {draftsLoading ? (
            ...
            <div className="bg-white rounded-md border border-zinc-100 px-4 py-5 text-center">
              <p className="text-sm text-zinc-400">No pending drafts</p>
            </div>
          ) : (
            <div className="space-y-3">
              {drafts.map((draft, i) => (
  ...
  return (
    <div className="h-full flex items-center justify-center bg-zinc-50">
      <div className="text-center max-w-xs px-6">
        <div className="w-10 h-10 bg-zinc-100 rounded-md mx-auto mb-4 flex items-center justify-center">
          <Zap className="w-4 h-4 text-zinc-400" />
        </div>
        <p className="text-[13px] font-medium text-zinc-700">Select a deal</p>
        <p className="text-[12px] text-zinc-400 mt-1.5 leading-relaxed">
          Pick any deal to see pending actions, AI drafts, and signals.
        </p>
        {pendingCount > 0 && (
          <div className="mt-4 text-[12px] text-zinc-400 flex items-center justify-center gap-1.5">
...
function InboxInner() {
  ...
  return (
    <div className="flex h-full">
      ...
      {/* ── Left panel ─────────────────────────────────────────────────── */}
      <div className="w-80 flex-shrink-0 border-r border-zinc-100 bg-white flex flex-col">
