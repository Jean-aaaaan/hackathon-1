    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50">
        <div className="text-center">
          <p className="text-lg font-semibold text-zinc-700">Link expired or invalid</p>
          <p className="text-sm text-zinc-400 mt-1">This brief link is no longer valid. Ask the sender for a new link.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-zinc-50">
      ...
      <div className="bg-white border-b border-zinc-200 px-6 py-4">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold text-zinc-700">Vantage · Account Brief</span>
          </div>
          <span className="text-xs text-zinc-400">Read-only · expires in 7 days</span>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-white rounded-xl border border-zinc-200 p-6">
          <h1 className="text-xl font-bold text-zinc-900">{brief.account_name}</h1>
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            {brief.stage && (
              <span className="text-sm text-zinc-500 bg-zinc-100 px-2.5 py-0.5 rounded-full">{brief.stage}</span>
            )}
            {brief.deal_amount && (
            ...
            {brief.ai_forecast && (
              <span className={cn("text-xs font-medium px-2.5 py-1 rounded-full flex items-center gap-1", FORECAST_STYLE[brief.ai_forecast] ?? "bg-zinc-100 text-zinc-600")}>
          ...
          {healthPct !== null && (
            <div className="mt-3 flex items-center gap-2">
              <span className="text-xs text-zinc-400">Account health</span>
              ...
              <span className="text-xs font-medium text-zinc-700">{healthPct}%</span>
            </div>
          )}
          {brief.meddpicc_overall != null && (
            <div className="mt-2 flex items-center gap-2">
              <span className="text-xs text-zinc-400">MEDDPICC</span>
              <div className="flex-1 h-1.5 bg-zinc-100 rounded-full overflow-hidden max-w-[120px]">
                <div
              ...
              <span className="text-xs font-medium text-zinc-700">{Math.round((brief.meddpicc_overall ?? 0) * 100)}%</span>
            </div>
          )}
        </div>
        ...
        {brief.top_signals.length > 0 && (
          <div className="bg-white rounded-xl border border-zinc-200 p-5">
            <h2 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-3">Key Signals</h2>
        ...
        {brief.next_actions.length > 0 && (
          <div className="bg-white rounded-xl border border-zinc-200 p-5">
            <h2 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-3">Recommended Actions</h2>
            <div className="space-y-2">
              {brief.next_actions.map((a, i) => (
                <div key={i} className="flex items-start gap-3 py-2">
                  <span className="w-5 h-5 rounded-full bg-zinc-100 text-zinc-700 text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
                    {i + 1}
                  </span>
                  ...
                  <span className={cn(
                    ...
                    a.urgency_score >= 0.7 ? "bg-orange-50 text-orange-600" : "bg-zinc-100 text-zinc-500"
                  )}>
              ...
              {brief.stakeholders.map((s, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="w-7 h-7 rounded-full bg-zinc-100 flex items-center justify-center flex-shrink-0">
                    <span className="text-xs font-bold text-zinc-700">{s.name[0]?.toUpperCase()}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-zinc-800">{s.name}</p>
                    <p className="text-xs text-zinc-400">{[s.title, s.role].filter(Boolean).join(" · ")}</p>
                  </div>
                  {s.sentiment && (
                    <span className={cn(
                      "text-xs px-1.5 py-0.5 rounded-full",
                      s.sentiment === "positive" ? "bg-green-100 text-green-700" :
                      s.sentiment === "negative" ? "bg-red-100 text-red-600" : "bg-zinc-100 text-zinc-500"
                    )}>
                      {s.sentiment}
                    </span>
                  )}
        ...
        {/* Footer */}
        <div className="text-center py-4 border-t border-zinc-200">
          <p className="text-xs text-zinc-400">
            Powered by <span className="font-semibold text-zinc-700">Vantage</span> · AI-generated account brief · Read-only
          </p>
        </div>
      </div>
    </div>
