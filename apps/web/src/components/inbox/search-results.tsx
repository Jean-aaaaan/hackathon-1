...
export function SearchResults({ query, results, isLoading, onSelect }: Props) {
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <p className="text-xs text-zinc-400 mb-6">Semantic search · Ranked by relevance × urgency</p>
        ...
        <div className="space-y-3">
          {results.map((account, i) => (
            <button
              ...
              className="w-full text-left bg-white border border-zinc-200 rounded-xl px-5 py-4 hover:border-brand-300 hover:shadow-sm transition-all"
            >
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm font-semibold text-zinc-900">{account.name}</p>
                  <p className="text-xs text-zinc-500 mt-0.5">{account.stage}</p>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className="text-xs text-zinc-400">Relevance</p>
              ...
              {account.signals_summary?.[0] && (
                <p className="text-xs text-zinc-500 mt-2 truncate">↑ {account.signals_summary[0].detail}</p>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="mt-4">
        <Link href="/inbox" className="text-xs text-zinc-400 hover:text-zinc-600">← Back to inbox</Link>
      </div>
    </div>
  );
}
